...
import { IdType, TaxRegime, type CustomerProps } from '@chax/shared';

export type { CustomerProps } from '@chax/shared';
...
export class Customer extends Entity<CustomerProps> {
    ...
    public static create(props: CustomerProps, id?: string): Customer {
        ...
        if (trimmedId.length === 0) throw new Error('El cliente debe tener un número de documento');
        if (trimmedId.length < 5 || trimmedId.length > 20) {
            throw new Error('El número de documento debe tener entre 5 y 20 caracteres');
        }
        const trimmedName = props.name?.trim() ?? '';
        if (trimmedName.length === 0) throw new Error('El cliente debe tener un nombre');
        if (trimmedName.length < 5) throw new Error('El nombre debe tener al menos 5 caracteres');
        const nameParts = trimmedName.split(/\s+/).filter(Boolean);
        if (nameParts.length < 2) throw new Error('Debe incluir al menos nombre y apellido');
