...
export class PrismaCustomerRepository implements ICustomerRepository {
    async existsByIdTypeAndIdNumberAndOrganization(
        ...
        const existing = await prisma.customer.findFirst({
            where: {
                organizationId,
                idType: idType as PrismaCustomer['idType'],
                idNumber,
    ...
    async findAllByOrganization(organizationId: string): Promise<Customer[]> {
        ...
        return list.map((c: PrismaCustomer) => this.mapToDomain(c));
    }
    ...
    private mapToDomain(prismaCustomer: PrismaCustomer): Customer {
        const props: CustomerProps = {
            idType: prismaCustomer.idType as CustomerProps['idType'],
            ...
            taxRegime: prismaCustomer.taxRegime as CustomerProps['taxRegime'],
