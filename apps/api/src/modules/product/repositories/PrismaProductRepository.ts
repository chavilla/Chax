...
type ProductInternalCode = { internalCode: string };
...
export class PrismaProductRepository implements IProductRepository {
    async getNextSequenceForPrefix(organizationId: string, prefix: string): Promise<number> {
        ...
        const numbers = (products as ProductInternalCode[])
            .map((p: ProductInternalCode) => {
    ...
    async findAllByOrganization(organizationId: string): Promise<Product[]> {
        ...
        return list.map((p: PrismaProduct) => this.mapToDomain(p));
    }
    ...
    private mapToDomain(prismaProduct: PrismaProduct): Product {
        const props: ProductProps = {
            ...
            taxType: prismaProduct.taxType as ProductProps['taxType'],
