...
type PrismaPurchase = NonNullable<Awaited<ReturnType<typeof prisma.purchase.findUnique>>>;
type PurchaseWithItemsArgs = { where: { id: string }; include: { items: true } };
type PurchaseWithItems = NonNullable<Awaited<ReturnType<typeof prisma.purchase.findUnique<PurchaseWithItemsArgs>>>>;
...
export class PrismaPurchaseRepository implements IPurchaseRepository {
    ...
    async findByIdWithItems(id: string): Promise<{ purchase: Purchase; items: PurchaseItemWithId[] } | null> {
        ...
        if (!row) return null;
        const purchase = this.mapToDomain(row as PrismaPurchase);
        const items: PurchaseItemWithId[] = (row as PurchaseWithItems).items.map((i: PurchaseWithItems['items'][number]) => ({
            id: i.id,
    ...
    async findAllByOrganization(organizationId: string): Promise<Purchase[]> {
        ...
        return list.map((r: PrismaPurchase) => this.mapToDomain(r));
    }
