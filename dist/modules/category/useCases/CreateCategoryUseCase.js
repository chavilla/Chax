...
let CreateCategoryUseCase = class CreateCategoryUseCase {
    ...
    async execute(request) {
        ...
        const exists = await this.categoryRepository.existsByNameAndOrganization(request.organizationId, request.name);
        if (exists) {
            throw new AppError_1.AppError(`Ya existe una categoría con el nombre "${request.name}" en esta organización`);
        }
