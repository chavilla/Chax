...
export class Supplier extends Entity<SupplierProps> {
    ...
    public static create(props: SupplierProps, id?: string): Supplier {
        ...
        if (trimmedIdNumber.length === 0) {
            throw new Error('El proveedor debe tener un número de documento');
        }
        ...
        if (trimmedName.length === 0) {
            throw new Error('El proveedor debe tener razón social');
        }
        if (trimmedName.length < 2) {
            throw new Error('La razón social debe tener al menos 2 caracteres');
        }
